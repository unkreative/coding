...
def replace(input:str):
    inputt = input
    replace = [["potage du", "Potage du"], ["Potage", "potage_"], ["\npotage", "potage_"], ["plat 1", "plat_1"], ["Plat 1", "plat_1"], ["plat 2", "plat_2"], ["Plat 2", "plat_2"], ["Légumes", "légumes_"], ["\ndessert", "dessert_"], ["Dessert", "dessert_"]]
            ...
            if (numc % 2) == 0:
                ...
                if self.subtitle['subtitle_present'][self.subtitle['subtitlelow_org'].index(x)] == False:
                    ...
                    try:
                        ...
                        p = self.jour.index(self.subtitle["subtitleupper_org"][self.subtitle["subtitlelow_org"].index(x)])
...
class Menu_du_internat:
    ...
    def internat_date(self):
        internat_day1 = self.day1[1]
        if self.days["days_present"][3] == True:
            internat_day2 = self.day4[1]
            internat_month = self.day4[2]

        elif self.days["days_present"][2] == True:
            internat_day2 = self.day3[1]
            internat_month = self.day3[2]
        
        year = get_year()
        self.date = f"menu du {internat_day1} au {internat_day2} {internat_month} {year}"
    ...
    def filter_plats(self): 
        daylow = self.days["daylow_org"]
        days_lst = [self.day1, self.day2, self.day3, self.day4]
        num1 = 0
        for x in daylow:
            ...
            if self.days["days_present"][self.days["daylow_org"].index(x)] == False:
                ...
                xmlday = []
                xmlday.append(f"""    <{days_lst[num1][0]}>\n""")
                xmlday.append(f"""        <title>{days_lst[num1][0]}</title>""")
                self.days = p.jour_lst
        ...
        <plat>{cache_plat}</plat>\n"""
                            ...
                            num2 += 1
                xmlday.append(f"""    </{days_lst[num1][0]}>\n""")
                p.update_presence_day(xmlday, days_lst[num1][0])
    ...
    def write_xml(self):
        xml_content = self.xml
        self.internat_date()
        f = open(f"{self.filename[0]}-internat.xml", "a")
        config = """<?xml version="1.0" encoding="UTF-8"?>\n"""
        menu_inst = """<internats_menu>\n"""
        menu_out = """</internats_menu>\n"""

        f.write(config)
        f.write(menu_inst)
        print("\n\n\nhere\n\n\n")
        f.write(f"    <date>{self.date}</date>\n")
        print("\n\n\nhere\n\n\n")
        print(self.xml)
        for x in self.xml:
            print(x)
    ...
    def change_xml(self):
        ...
        with open(f'{self.filename[0]}-internat.xml', 'r') as file:
            filedata = file.read()
        ...
        for thing in self.replace_in_xml:
            ...
            # Write the file out again
        with open(f'{self.filename[0]}-internat.xml', 'w') as file:
            file.write(filedata)
    ...
    def automate(self):
        def import_xml():
            ...
            time.sleep(2)
            print("filenameee ", self.filename)
            write_to_clipboard(f"{self.filename[0]}-internat.xml")
            pg.hotkey("command", "v")
            time.sleep(2)
            pg.press("enter")
            # pg.click(1548, 420)
        ...
        # set directories 
        self.final_dir_pdf = f'{self.menu_folder_path}/{self.filename[0]} internat.pdf'
        self.final_dir_id = f'{self.menu_folder_path}/{self.filename[0]} internat indesign'
        print()
        print()
        print("aa")
        print()
...
internat_days = {
    "original_days":['lundi', 'mardi', 'mercredi', 'jeudi'],
    "check_days":['lundi', 'mardi', 'mercredi', 'jeudi'],
    "daylow_org":['lundi', 'mardi', 'mercredi', 'jeudi'],
    "dayupper_org":['Lundi', 'Mardi', 'Mercredi', 'Jeudi'],
    "days_present":['lundi', 'mardi', 'mercredi', 'jeudi']
}
internat_plats = {
    "original_plats":['potage', 'plat1', 'plat2', 'accompagnement', 'légumes', 'dessert'],
    "check_plats":['potage_', 'plat_1', 'plat_2', 'accompagnement', 'légumes_', 'dessert_'],
...
def main():
    ...
    menu.replace_in_xml = [['potage_', "potage"], ['plat_1', "plat 1"], ['plat_2', "plat 2"], ['légumes_', "légumes"], ['dessert_', "dessert"]]


    internat = Menu_du_internat()

    internat.days = internat_days
    internat.plats = internat_plats
    internat.subtitle = internat_subtitle

    internat.days_indexes = []
    internat.day1 = []
    internat.day2 = []
    internat.day3 = []
    internat.day4 = []
    ...
    internat.replace_in_xml = [['potage_', "potage"], ['plat_1', "plat 1"], ['plat_2', "plat 2"], ['légumes_', "légumes"], ['dessert_', "dessert"]]

    internat.indesign_file = '/Users/NLNEW/coding/indesign templates/internat menu.indt'
    
    internat.txt_placeholder = "PLACEHOLDER"
    ...
    def run_menu():
        ...
        root.filename = filedialog.askopenfilename(initialdir="Volumes/Classes_21_22/Chelsea Studios/1 Graphic Designers/2 Clients/16 Restaurant Mélusine/", title="select a file",
                                                   filetypes=filetypes)
        ...
        ttk.Label(root, text=f"menu number: {menu.filename[0]}").grid(row=8, column=menu_col)
    ...
    def run_internat():
        ...
        menu_folder_path = originfile_name.rsplit("/", 1)
        menu_folder_path = menu_folder_path[0][1:]
        internat.menu_folder_path = menu_folder_path
        print(internat.menu_folder_path)

        internat.input = replace(internat_file(originfile_name))
        print(internat.input)
        internat.check_input()

        internat.wordsToList()
        internat.split_days()

        internat.filter_plats()
        internat.get_filename()
        print(internat.input)
        ...
        ttk.Label(root, text=f"menu number: {internat.filename[0]}").grid(row=8, column=menu_col)
    ...
    menu_filebutton = ttk.Button(root, text="open file", command=run_menu)
    ...
    internat_filebutton = ttk.Button(root, text="open file", command=run_internat) # command=run_internat
...
def main_xml():
    ...
    menu.indesign_file = '/Users/NLNEW/coding/menu du jour.indt'
    menu.txt_placeholder = "PLACEHOLDER"
    ...
    internat.indesign_file = '/Users/NLNEW/coding/internat menu.indt'
    internat.txt_placeholder = "PLACEHOLDER"
    ...
    def run_menu():
        ...
        filename = filedialog.askopenfilename(initialdir="Volumes/Classes_21_22/Chelsea Studios/1 Graphic Designers/2 Clients/16 Restaurant Mélusine/", title="select a file",
                                                   filetypes=filetypes)
    ...
    inp = input("enter mode (menu, internat): ")
    if inp == "menu":
        run_menu()
    elif inp == "internat":
        run_internat()
