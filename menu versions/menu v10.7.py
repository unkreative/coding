...
def remove_internat(input, index_days):
    ...
    print(input)
    num1 = 0
    for x in internat_menu:
        internat_menu[num1] = x.replace("Internat", "\n")
        num1 += 1

    with open(filepath_internat, "w") as finternat:
        for item in internat_menu:
            finternat.write(f"{item}")
...
class Menu_du_jour:
    ...
    def check_input(self):
        ...
        if "." in self.input:
            nums_month = ["01", "02", "03", "04", "05", "06", "07", "08", "09", "10", "11", "12"]
    ...
    def split_days(self):
        ...
        self.input, it = remove_internat(self.input, self.days_indexes)
        print(it)
    ...
    def filter_plats_html(self):
        ...
        for x in daylow:
            ...
            if self.days["days_present"][self.days["daylow_org"].index(x)] == False:
                ...
                num2 = 0
                xmlday = []
                xmlday.append(f"""    <{days_lst[num1][0]} class='main_div'>""")
                date = f"""\n        <date class='date'>{p.jour[0]} {p.jour[1]} {p.jour[2]} {get_year()}</date>\n
                ...
                if self.days["days_present"][num1] == False:
                    ...
                    for x in plats_lst:
                        if x:
                            ...
                            module1 = f"""
        <div id="{self.txt_placeholder}_{cache_title}">
            <h4 class="title">{cache_title}</h4>
            <h2 class="plat">{cache_plat}</h2>
            <h5 class="undertitle">{subtitle_cache}</h5>
        </div>\n"""
                            xmlday.append(module1)

                            num2 += 1
                xmlday.append(f"""
                    <img src="/../web/lem_menu/root/images/restaurant-menu_title.png" width="793" alt="Subtitle" class='sub_div'>

                    </{days_lst[num1][0]}>\n""")
                # p.update_presence_day(xmlday, days_lst[num1][0], date)

                self.html.append(xmlday)
                print()
                print()
                print(days_lst[num1][0])
                print()
    ...
    def write_html(self):
        xml_content = self.html

        f = open(f"/Users/lousergonne/coding/{self.filename[0]}.html", "a")
        config = """
    <style>
        /* for the menu title */
                .title {
...
"""
    ...
    def change_html(self):
        ...
        with open(f'/Users/lousergonne/coding/{self.filename[0]}.html', 'r') as file:
            filedata = file.read()
        ...
        for thing in self.replace_in_xml:
            ...
            # Write the file out again
        with open(f'/Users/lousergonne/coding/{self.filename[0]}.html', 'w') as file:
            file.write(filedata)
